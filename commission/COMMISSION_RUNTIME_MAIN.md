...
5. `pool` นำ `PV` ของ order ที่อนุมัติแล้วเข้าระบบคำนวณทันที
6. สินค้า `FIRM` เมื่อสั่งซื้อแบบปกติและ order ถูกอนุมัติแล้ว จะได้รับ `Firm wallet` แบบ `1:1 บาท` ตามยอดที่กำหนด
7. `matrix auto order` ได้รับ `Firm wallet` ตามยอด auto order และอนุมัติทันทีตามกติกา
8. `PV` ที่ถูกใช้ใน `matrix` จะเข้าสู่การสะสมและถูกใช้ตามรหัส/รอบของระบบ ไม่ถูกนำกลับมาใช้ซ้ำในรหัสเดิม
...
5. ระบบสร้าง payout ของตำแหน่งนั้นตาม level/rate ของจุดที่ลง
...
5. สร้าง matrix payout ให้ผู้รับผลของ slot นั้น
