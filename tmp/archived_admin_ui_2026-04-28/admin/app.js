...
const endOfDayProcessButton = document.getElementById("endOfDayProcessButton");
...
state.activeAdminMenu =
  (localStorage.getItem("adminActiveMenu") || "overview") === "marketing"
    ? "members"
...
const ACTIVE_COMMISSION_TYPES = new Set([
  "direct",
  "team_2leg",
  "team_3leg",
  "matching_l1",
...
const COMMISSION_REPORT_GROUPS = {
  direct: new Set(["direct"]),
  team: new Set(["team_2leg", "team_3leg"]),
  matching: new Set(["matching_l1", "matching_l2"]),
  pool: new Set(["pool"]),
};
...
const adminMenuConfig = {
  ...
  members: {
    title: "ข้อมูลสมาชิก / Commission Report",
    description:
      "Member records, active commission channels, profile lookup, and network-related operations.",
  },
  line: {
    title: "จัดการ LINE",
    description:
...
function isActiveCommissionType(value) {
  return ACTIVE_COMMISSION_TYPES.has(normalizeCommissionType(value));
}

function matchesCommissionGroup(value, selectedGroup) {
  if (!selectedGroup) {
    return true;
  }

  const normalizedType = normalizeCommissionType(value);
  const allowedTypes = COMMISSION_REPORT_GROUPS[selectedGroup];

  return allowedTypes ? allowedTypes.has(normalizedType) : true;
}

function getCommissionReportLabel(value) {
  const normalizedType = normalizeCommissionType(value);

  if (normalizedType === "team_2leg") {
    return "Team 2-Leg";
  }
  if (normalizedType === "team_3leg") {
    return "Team 3-Leg";
  }
  if (normalizedType === "matching_l1") {
    return "Matching L1";
  }
  if (normalizedType === "matching_l2") {
    return "Matching L2";
  }
  if (normalizedType === "direct") {
    return "Direct";
  }
  if (normalizedType === "pool") {
    return "Pool";
  }
...
async function loadDashboard() {
  ...
  const activeCommissionItems = commissionItems.filter((commission) =>
    isActiveCommissionType(commission.commissionType),
  );
  const visibleCommissionItems = activeCommissionItems.filter((commission) =>
    matchesCommissionGroup(commission.commissionType, state.commissionType),
  );
  ...
  state.commissionItems = activeCommissionItems;
  ...
  state.totals.commissions = visibleCommissionItems.length;
  state.latestOrderId = orderItems[0]?.orderId || "";
  state.latestCommission = activeCommissionItems[0] || null;
...
async function processEndOfDaySettlement(settlementDate) {
  if (!settlementDate) {
    setActionOutput("End-of-day process failed", {
      message: "settlementDate is required",
    });
    return;
  }

  setStatus(`Processing end of day ${settlementDate}`);
  const result = await request(
    `/commissions/end-of-day/${encodeURIComponent(settlementDate)}/process`,
    { method: "POST" },
  );
  setActionOutput(`End-of-day process ${settlementDate}`, result);
  setStatus(`Processed end of day ${settlementDate}`);
  pushHistory("End Of Day", `Processed end-of-day commission batch for ${settlementDate}`);
}
...
if (commissionTypeFilter) {
  commissionTypeFilter.addEventListener("change", (event) => {
    state.commissionType = String(event.target.value || "").trim().toLowerCase();
    state.pages.commissions = 1;
    loadDashboard().catch((error) => setStatus(error.message));
  });
}
...
if (endOfDayProcessButton) {
  endOfDayProcessButton.addEventListener("click", () => {
    processEndOfDaySettlement(teamSettlementDateInput?.value || "").catch((error) => {
      setStatus(error.message);
      setActionOutput("End-of-day process failed", { message: error.message });
    });
  });
}
...
(async function bootstrap() {
  if (state.activeAdminMenu === "marketing") {
    state.activeAdminMenu = "members";
    localStorage.setItem("adminActiveMenu", "members");
  }
