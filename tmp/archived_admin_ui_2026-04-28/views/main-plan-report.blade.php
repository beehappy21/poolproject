@php
    ...
    $liveSummaryCards = $commissionMainPlanLiveSummaryCards ?? [];
    $section = $commissionSection ?? [];
    $mainPlanMeta = $commissionMainPlanMeta ?? [];
...
<div class="commission-shell">
    <section class="commission-main">
        <div class="commission-panel">
            <div class="commission-eyebrow">{{ $section['eyebrow'] ?? 'Commission Main Plan Report' }}</div>
            <h1 class="commission-title">{{ $section['title'] ?? 'รายงานคอมมิชชั่นแผนหลัก' }}</h1>
            <p class="commission-description">{{ $section['description'] ?? 'พื้นที่แสดงผล BAO ชุดใหม่สำหรับงาน commission main plan โดยแยกจากรายงานเดิม' }}</p>
            <div class="commission-note" style="margin-top:1rem;">
                <strong>Main plan basis:</strong>
                เริ่มคำนวณจาก <code>{{ $mainPlanMeta['calcStart'] ?? 'approved_at' }}</code>
                ครอบคลุม <code>{{ $mainPlanMeta['scope'] ?? 'Direct / Pool / Matrix' }}</code>
                และใช้ route แยกของงานนี้เท่านั้น
                (<code>{{ $mainPlanMeta['routeIsolation'] ?? 'isolated main-plan report' }}</code>)
                ล่าสุดที่นับได้ <code>{{ $mainPlanMeta['latestApprovedAt'] ?? '-' }}</code>
            </div>
        ...
        @if ($liveSummaryCards !== [])
            <div class="commission-card-grid">
                @foreach ($liveSummaryCards as $card)
                    <article class="commission-panel commission-card">
                        ...
                        <div class="commission-card__value">
                            @if (($card['format'] ?? 'decimal') === 'count')
                                {{ number_format((float) $card['value'], 0, '.', ',') }}
                            @elseif (($card['format'] ?? 'decimal') === 'text')
                                {{ $card['value'] }}
                            @else
