...
type MatrixPayoutResponse = MatrixPayoutSummary[] | {items?: MatrixPayoutSummary[]};

type PoolPayoutSummary = {
  payoutId: string;
  poolDate: string;
...
const defaultMainPlanSummary: MainPlanSummary = {
  totalEntries: 0,
  approvedEntries: 0,
  fallbackEntries: 0,
  ledgerAmount: '0.00',
...
const resolveBoardDepth = (
  ...
  const slotCount = normalizePositiveInteger(board.slotCount, 0);

  if (slotCount > 0) {
    let runningTotal = 0;

    for (let level = 1; level <= 12; level += 1) {
      runningTotal += boardWidth ** level;
      if (runningTotal >= slotCount) {
        return level;
      }
    }
  }

  return board.boardNo === 1 ? 3 : 2;
};

const getBoardLevelStartSlot = (
  cycle: MatrixCycleSummary,
  levelNo: number,
): number => {
  const boardWidth = normalizePositiveInteger(cycle.boardWidth, 2);
...
const extractMatrixPayoutItems = (
  payload?: MatrixPayoutResponse,
): MatrixPayoutSummary[] => {
  if (!payload) {
    return [];
  }
...
const isApprovedLikeCommissionEntry = (entry: CommissionEntry) => {
  const status = (entry.status || '').toLowerCase();
  return status !== 'fallback' && status !== 'reversed';
};

const isApprovedLikePoolPayout = (payout: PoolPayoutSummary) => {
  const status = (payout.status || '').toLowerCase();
  return status !== 'fallback' && status !== 'reversed';
};

const isApprovedLikeMatrixPayout = (payout: MatrixPayoutSummary) => {
  const status = (payout.status || '').toLowerCase();
  return status !== 'reset';
};
...
export const CommissionMainPlan: React.FC = () => {
  ...
  const [matrixPayouts, setMatrixPayouts] = useState<MatrixPayoutSummary[]>([]);
  const [poolPayouts, setPoolPayouts] = useState<PoolPayoutSummary[]>([]);
  ...
  const [mainPlanSummary, setMainPlanSummary] = useState<MainPlanSummary>(
    defaultMainPlanSummary,
  );
  ...
  const loadCommissionPage = async () => {
    ...
    try {
      const [
        ...
        axios.get<MatrixPayoutResponse>(URLS.AUTH_MATRIX_PAYOUTS, authRequestConfig),
        axios.get<PoolPayoutSummary[]>(URLS.AUTH_POOL_PAYOUTS, authRequestConfig),
      ]);
      ...
      const resolvedMatrixPayouts =
        matrixPayoutsResult.status === 'fulfilled'
          ? extractMatrixPayoutItems(matrixPayoutsResult.value.data)
          : [];
      setMatrixPayouts(resolvedMatrixPayouts);

      const resolvedPoolPayouts =
        poolPayoutsResult.status === 'fulfilled' ? poolPayoutsResult.value.data : [];
      setPoolPayouts(resolvedPoolPayouts);

      const approvedEntries = commissions.filter(isApprovedLikeCommissionEntry).length;
      const fallbackEntries = commissions.filter(
        entry => (entry.status || '').toLowerCase() === 'fallback',
      ).length;
      const ledgerAmount = commissions
        .filter(isApprovedLikeCommissionEntry)
        .reduce((sum, entry) => sum + parseDecimal(entry.amount), 0);
      const matrixAmount = resolvedMatrixPayouts
        .filter(isApprovedLikeMatrixPayout)
        .reduce((sum, payout) => sum + parseDecimal(payout.amount), 0);
      const poolAmount = resolvedPoolPayouts
        .filter(isApprovedLikePoolPayout)
        .reduce((sum, payout) => sum + parseDecimal(payout.payoutAmount), 0);
      const latestCreatedAt = [
        ...commissions
          .filter(isApprovedLikeCommissionEntry)
          .map(entry => entry.createdAt || ''),
        ...resolvedMatrixPayouts
          .filter(isApprovedLikeMatrixPayout)
      ...
      setMainPlanSummary({
        ...
        approvedEntries,
        fallbackEntries,
        ledgerAmount: formatDecimal(ledgerAmount),
        matrixAmount: formatDecimal(matrixAmount),
        poolAmount: formatDecimal(poolAmount),
        latestApprovedLabel: latestCreatedAt ? formatDateTime(latestCreatedAt) : '-',
      });
      ...
      const todayMatrixTotal = resolvedMatrixPayouts.reduce((sum, payout) => {
        const createdAt = new Date(payout.createdAt || '').getTime();

        if (
          Number.isNaN(createdAt) ||
          createdAt < startOfToday ||
          !isApprovedLikeMatrixPayout(payout)
        ) {
          return sum;
        }
      ...
      setMainPlanSummary(defaultMainPlanSummary);
  ...
  const unilevelEntries = useMemo(() => {
    return commissionEntries
      .filter(entry => entry.commissionType?.toLowerCase() === 'uni')
      .sort(
        (left, right) =>
          new Date(right.createdAt || 0).getTime() -
          new Date(left.createdAt || 0).getTime(),
  ...
  const unilevelTotal = useMemo(() => {
    return unilevelEntries.reduce((sum, entry) => sum + parseDecimal(entry.amount), 0);
  }, [unilevelEntries]);
  ...
  const matrixPayoutTotal = useMemo(() => {
    return matrixPayouts
      .filter(isApprovedLikeMatrixPayout)
      .reduce((sum, payout) => sum + parseDecimal(payout.amount), 0);
  }, [matrixPayouts]);
  ...
  const renderCommissionSummaryCard = () => {
    ...
    const selectedCardNote = selectedCard.note;
      ...
      return (
        <section style={{display: 'grid', gap: 12}}>
          ...
          >
            ...
            >
              รวม {formatDecimal(unilevelTotal)}
            </h3>
            <p style={{margin: 0, color: theme.colors.textColor, lineHeight: 1.6}}>
              รายการ unilevel จริง {unilevelEntries.length} รายการ
            </p>
          </div>

          {unilevelEntries.slice(0, 5).map(entry => (
            <article
              key={entry.commissionId || `${entry.createdAt}-${entry.amount}`}
              style={{
                padding: 14,
            ...
            >
              ...
              >
                <div style={{color: theme.colors.mainColor, ...theme.fonts.Mulish_700Bold}}>
                  Unilevel {entry.levelNo ? `L${entry.levelNo}` : ''}
                </div>
                <div style={{color: theme.colors.mainColor, ...theme.fonts.Mulish_700Bold}}>
                  {formatDecimal(parseDecimal(entry.amount))}
                </div>
          ...
          {!unilevelEntries.length ? (
            <div style={{color: theme.colors.textColor}}>ยังไม่มีข้อมูล unilevel</div>
          ) : null}
        </section>
      );
    }

    if (selectedCard.key === 'pool') {
      return (
        <section style={{display: 'grid', gap: 12}}>
          <div
            style={{
          ...
          >
            ...
            >
              รวม {formatDecimal(poolTotal)}
            </h3>
            <p style={{margin: 0, color: theme.colors.textColor, lineHeight: 1.6}}>
              รายการ pool payout จริง {poolPayouts.length} รายการ
            </p>
          </div>

          {poolPayouts.slice(0, 5).map(payout => (
            <article
              key={payout.payoutId}
              style={{
                padding: 14,
            ...
            >
              ...
              >
                <div style={{color: theme.colors.mainColor, ...theme.fonts.Mulish_700Bold}}>
                  Pool {payout.poolDate}
                </div>
                <div style={{color: theme.colors.mainColor, ...theme.fonts.Mulish_700Bold}}>
                  {formatDecimal(parseDecimal(payout.payoutAmount))}
                </div>
              </div>
              <div style={{color: theme.colors.textColor, lineHeight: 1.6}}>
                <div>เวลา: {formatDateTime(payout.createdAt)}</div>
                <div>สถานะ: {payout.status || '-'}</div>
                {payout.blockReason ? <div>เหตุผล: {payout.blockReason}</div> : null}
              </div>
            </article>
          ))}

          {!poolPayouts.length ? (
            <div style={{color: theme.colors.textColor}}>ยังไม่มีข้อมูล pool payout</div>
          ) : null}
        </section>
      );
    }
  ...
  const getBoardLevelRows = (cycle: MatrixCycleSummary, board: MatrixBoardSummary) => {
    const boardDepth = resolveBoardDepth(cycle, board);
    ...
    return Array.from({length: boardDepth}, (_, index) => {
      ...
      const positions = (board.positions || [])
        ...
        .sort((left, right) => (left.slotNo || 0) - (right.slotNo || 0));
      const capacity = getBoardLevelCapacity(cycle, levelNo);
      const startSlot = getBoardLevelStartSlot(cycle, levelNo);
      const filled = positions.length;
      const percent = Math.max(0, Math.min(100, (filled / capacity) * 100));
      const slotItems = Array.from({length: capacity}, (_, slotIndex) => {
        const slotNo = startSlot + slotIndex;
        const position =
          positions.find(entry => (entry.slotNo || 0) === slotNo) ||
          positions[slotIndex] ||
          null;
...
const getBoardStatusLabel = (board: MatrixBoardSummary) => {
      ...
      >
        ...
        <div
          style={{
            display: 'grid',
            gap: isMobileViewport ? 6 : 8,
            maxHeight: isMobileViewport ? 122 : 208,
            overflowY: 'auto',
            paddingRight: 2,
          }}
        >
          {levelRows.map(level => (
            <div key={`${board.boardId}-level-${level.levelNo}`}>
              ...
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: `repeat(${level.capacity}, minmax(0, 1fr))`,
                  gap: isMobileViewport ? 4 : 5,
                  marginTop: isMobileViewport ? 4 : 6,
                }}
              >
                {level.slotItems.map(slot => {
                  const memberCode = slot.position?.sourceMemberCode || '';
                  const shortCode = memberCode ? memberCode.slice(-4) : '';
                  const isFilled = Boolean(slot.position);

                  return (
                    <div
                      key={`${board.boardId}-slot-${slot.slotNo}`}
                      style={{
                        minHeight: isMobileViewport ? 28 : 32,
                        borderRadius: 10,
                        border: isFilled
        ...
        >
          ...
          >
            <div>
              ...
              >
                แสดง flow ของ slot จริงในกระดานนี้แบบ 1 แตก 2 ตามแต่ละชั้น พร้อมรายชื่อสมาชิกที่ลงตำแหน่งแล้ว
              </p>
            </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          style={{
                            marginTop: 4,
                            color: isFilled ? '#1E3A8A' : '#94A3B8',
                            fontSize: 11,
                            lineHeight: 1.3,
                            ...theme.fonts.Mulish_600SemiBold,
                          }}
                        >
                          {slot.position?.sourceMemberCode || 'ว่าง'}
                        </div>
                      </div>
                    );
                  })}
  ...
  const renderMatrixPayoutPanel = (): JSX.Element => {
    return (
      <section style={{display: 'grid', gap: 12, marginTop: 16}}>
        ...
        >
          ...
          >
            รวม {formatDecimal(matrixPayoutTotal)}
          </h3>
          <p style={{margin: 0, color: theme.colors.textColor, lineHeight: 1.6}}>
            รายการ matrix payout จริง {matrixPayouts.length} รายการ
          </p>
        </div>

        {matrixPayouts.slice(0, 5).map(payout => (
          <article
            key={payout.payoutId}
            style={{
              padding: 14,
          ...
          >
            ...
            >
              <div style={{color: theme.colors.mainColor, ...theme.fonts.Mulish_700Bold}}>
                Board {payout.boardNo || '-'} {payout.levelNo ? `L${payout.levelNo}` : ''}
              </div>
              <div style={{color: theme.colors.mainColor, ...theme.fonts.Mulish_700Bold}}>
                {formatDecimal(parseDecimal(payout.amount))}
              </div>
            </div>
            <div style={{color: theme.colors.textColor, lineHeight: 1.6}}>
              <div>เวลา: {formatDateTime(payout.createdAt)}</div>
              <div>สถานะ: {payout.status || '-'}</div>
              <div>PV: {formatDecimal(parseDecimal(payout.basePv))}</div>
            </div>
          </article>
        ))}

        {!matrixPayouts.length ? (
          <div style={{color: theme.colors.textColor}}>ยังไม่มีข้อมูล matrix payout</div>
        ) : null}
      </section>
    );
  };
  ...
  return (
    <>
      <components.Header title='คอมมิชชั่นหลัก / Commission Main' goBack={true} />
      <main style={{padding: '20px 20px 120px'}}>
        ...
        >
          ...
          >
            คำนวณจากรายการที่อนุมัติแล้วเท่านั้น และหน้าจอนี้เป็นสำเนาแยกสำหรับงาน main plan
          </p>
          <div
            style={{
              display: 'grid',
          ...
          >
            {[
              {
                label: 'All Entries',
                value: String(mainPlanSummary.totalEntries),
              },
              {
                label: 'Approved',
                value: String(mainPlanSummary.approvedEntries),
              },
              {
                label: 'Fallback',
                value: String(mainPlanSummary.fallbackEntries),
              },
              {
                label: 'Ledger',
                value: mainPlanSummary.ledgerAmount,
              },
              {
                label: 'Matrix',
                value: mainPlanSummary.matrixAmount,
              },
              {
                label: 'Pool',
                value: mainPlanSummary.poolAmount,
              },
              {
                label: 'Latest Approved',
                value: mainPlanSummary.latestApprovedLabel,
              },
            ].map(item => (
              <div
                key={item.label}
              ...
              >
                ...
                <div
                  style={{
                    fontSize: item.label === 'Latest Approved' ? 12 : 20,
                    lineHeight: 1.3,
                    ...theme.fonts.Mulish_700Bold,
                  }}
                >
