...
class WithdrawRequestDetailScreen extends Screen
{
    ...
    public function commandBar(): iterable
    {
        return [
            ...
            Button::make('Approve')
                ...
                ->canSee(in_array($this->withdrawRequest->status, ['PENDING', 'REJECTED'], true)),
            Button::make('Reject')
                ->icon('bs.x-circle')
                ->method('rejectRequest')
                ->canSee(in_array($this->withdrawRequest->status, ['PENDING', 'APPROVED'], true)),
            Button::make('Mark Paid')
                ->icon('bs.bank')
                ->method('markPaid')
                ->canSee(in_array($this->withdrawRequest->status, ['APPROVED', 'EXPORTED'], true)),
        ];
    }
    ...
    public function layout(): iterable
    {
        return [
            Layout::rows([
                ...
                Input::make('request.tax_amount')->title('Tax')->readonly(),
                Input::make('request.auto_sweep_amount')->title('Auto sweep')->readonly(),
                Input::make('request.fee_amount')->title('Fee')->readonly(),
                Input::make('request.net_bank_amount')->title('Net bank amount')->readonly(),
    ...
    {
        $this->withdrawRequest->forceFill([
            'status' => 'APPROVED',
            'approvedAt' => now(),
