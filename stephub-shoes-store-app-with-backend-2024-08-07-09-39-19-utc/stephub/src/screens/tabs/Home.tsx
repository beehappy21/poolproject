...
export const Home: FC = () => {
  ...
  const renderBestSellers = (): JSX.Element => {
    ...
    const displayProducts = popularProducts.length ? popularProducts : productsData;
      ...
      <div style={{marginBottom: 40, display: 'flex', flexDirection: 'column'}}>
        <components.BlockHeading
          ...
          viewAllOnClick={() => {
            ...
            navigate('/Shop', {
              state: {products: displayProducts, title: 'สินค้าแนะนำ'},
            });
          }}
        />
        <custom.ScrollView style={{paddingLeft: 20, paddingRight: 20}}>
          {displayProducts?.map((item: any, index: number, arra: any) => {
      ...
      <div style={{marginBottom: 40, display: 'flex'}}>
        <button
          onClick={() => {
            dispatch(actions.resetFilters());
            navigate('/Shop', {
              state: {products: matches, title: 'สินค้าแนะนำ'},
            });
          }}
        >
          <img
  ...
  const renderFeatured = (): JSX.Element => {
    const featuredProducts = productsData.filter((item: any) => item.isFeatured);
    const displayProducts = featuredProducts.length ? featuredProducts : productsData;
      ...
      <div style={{marginBottom: 40, display: 'flex', flexDirection: 'column'}}>
        <components.BlockHeading
          ...
          viewAllOnClick={() => {
            dispatch(actions.resetFilters());
            navigate('/Shop', {
              state: {products: displayProducts, title: 'สินค้าเด่น'},
            });
          }}
        />
        <custom.ScrollView style={{paddingLeft: 20, paddingRight: 20}}>
    ...
    if (!productsData.length) {
      return (
        ...
        >
          ...
          >
            ยังไม่มีสินค้าพร้อมขายในขณะนี้ กรุณากลับมาตรวจสอบอีกครั้งภายหลัง
          </div>
        </main>
      );
    }
